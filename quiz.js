...
function generateHtmlQuestionsString(state, question){
	const currentQuestion = question[state.questionNumber].question;
	const options = question[state.questionNumber].options;
  
	return `<div class="question-page">
    <div class="quiz-info-section">
        <p>Question <span class="current-question-number">${state.questionNumber + 1}</span> of <span class="total-question-number">${QUESTIONS.length}</span></p>
...
function handleNextQuestion(){
	$('.js-main').on('click', '.js-next-question-button', function(){
		if (STORE.questionNumber < QUESTIONS.length){
			...
			STORE.correctAnswer = QUESTIONS[STORE.questionNumber].correctAns;
      STORE.currentView = 'quiz';
      STORE.questionNumber = STORE.askedQuestions[STORE.indexTracker];
      STORE.indexTracker++;
			renderHtml();
		} else {
